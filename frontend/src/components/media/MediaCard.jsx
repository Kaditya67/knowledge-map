import { Star, Edit, Trash2, ExternalLink, Play, CheckCircle, CheckSquare, Square } from "lucide-react"
import Badge from "../ui/Badge"
import { useAuth } from "../../context/AuthContext"

export default function MediaCard({ item, config, onEdit, onDelete, onIncrement, onToggleFavorite, viewMode = "grid", selectionMode = false, isSelected = false, onSelect }) {
  const { user } = useAuth()
  ...
  if (viewMode === "list") {
    return (
      <div className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 hover:shadow-lg transition-all duration-300 flex items-center gap-4 relative">
        {/* Selection Checkbox */}
        {selectionMode && (
          <button
    ...
    <>
      ...
      {item.coverImage && (
        <div className="relative w-full h-32 bg-slate-100 dark:bg-slate-800 overflow-hidden">
          <img src={item.coverImage} alt={item.title} className="w-full h-full object-contain" />
          {user && item.favorite && (
            <div className="absolute top-2 right-2 w-6 h-6 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-full flex items-center justify-center">
      ...
      {/* Selection Checkbox - Always show when in selection mode */}
      {selectionMode && (
        <div className={`absolute top-2 left-2 z-10`}>
          <button
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); onSelect(); }}
            className="w-6 h-6 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-md flex items-center justify-center hover:bg-white dark:hover:bg-slate-800 transition-colors border border-slate-300 dark:border-slate-600"
          >
            {isSelected ? (
      ...
      <div className="p-3">
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-sm font-bold text-slate-900 dark:text-white line-clamp-2 flex-1 pr-2" title={item.title}>
            {item.title}
          </h3>
          ...
          {user && (
            <div className="flex gap-1 shrink-0">
              ...
              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onDelete(item); }} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg text-red-500">
        ...
        <div className="flex items-center gap-1.5 mb-2 flex-wrap">
          <Badge color={statusObj?.color || "#94a3b8"}>{statusObj?.label || item.status}</Badge>
          {item.rating && <span className="text-[10px] font-bold text-amber-500">★ {item.rating}</span>}
        </div>
        ...
        {item.genres && item.genres.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-2">
            {item.genres.slice(0, 2).map(genre => (
              <span key={genre} className="text-[10px] px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-full">
                {genre}
              </span>
            ))}
            {item.genres.length > 2 && (
              <span className="text-[10px] px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-full">
                +{item.genres.length - 2}
              </span>
            )}
          </div>
          ...
          <div className="flex justify-end">
            {item.status !== "completed" ? (
              <button 
                ...
                className="w-full flex items-center justify-center gap-1.5 text-[11px] font-bold text-white bg-indigo-600 hover:bg-indigo-700 py-2 rounded-lg transition-colors shadow-sm shadow-indigo-500/20"
              >
                <Play className="w-3 h-3 fill-current" />
                +1 {typeConfig.unit}
              </button>
            ) : (
              <div className="w-full text-center py-2 text-[11px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 rounded-lg flex items-center justify-center gap-1.5">
  ...
  if (item.link) {
    return (
      <a 
        ...
        className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 block relative"
      >
        {cardContent}
      </a>
    );
  ...
  return (
    <div className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative">
      {cardContent}
    </div>
  );
}
